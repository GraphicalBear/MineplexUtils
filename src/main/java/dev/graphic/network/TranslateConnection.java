...
public abstract class TranslateConnection {
	...
	private static String getLanguage(String languageInput) throws Exception {
		String temp = getData(rootURL + detReq + authKey + text + languageInput)
				.toString();
		temp = temp
				.substring(temp.lastIndexOf(':') + 2, temp.lastIndexOf("\""));
		if (!temp.equals(""))
			return temp;
		else
			return (userLanguage);
	...
	private static String getTranslation(String source, String sourceLang,
			String targetLang) throws Exception {
		if (sourceLang == targetLang)
			return "Cannot be translated";

		String temp = getData(
				rootURL + transReq + authKey + lang + sourceLang + "-"
						+ targetLang + text + source).toString();
	...
	private static StringBuffer getData(String url) throws Exception {
		URL yandex;
		yandex = new URL(url);
		HttpURLConnection connection = (HttpURLConnection) yandex
				.openConnection();
	...
	public static void captureLanguages() throws Exception {
		String temp = getData(
				rootURL + lngReq + authKey + lngDisp + userLanguage).toString()
				.toLowerCase();
		langCodes = temp
				.substring(temp.indexOf('[') + 1, temp.indexOf(']') - 1);
		langDisps = temp.substring(temp.indexOf("langs") + 8,
				temp.indexOf('}') - 2);
	}

	public static String setLanguage(String lang) {
		if (langDisps.contains(lang) && lang.length() > 2) {
			userLanguage = langDisps.substring(langDisps.indexOf(lang) - 5,
					langDisps.indexOf(lang) - 4);
			return lang;
		} else
			userLanguage = "en";
		return "English";
